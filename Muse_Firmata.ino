...
const int DIGI_WRITE = 32;	//pins 2-15
const int DIGI_WRITE_2 = 24;    //pins 16-19
...
void loop() {
  ...
  if (stringComplete&&pntr) {
    for(int i=0; i<pntr; i++){
      ...
      if(readMethod){
        if(cur==192 && i+1<pntr){  //wire feed through
          int nex = input[++i];
          Wire.beginTransmission(nex);
          int cnt = 0;
          while(input[++i]!=160&&i<pntr){
            cnt++;
            Wire.write(input[i]);
          }
          if(input[i]==160) readMethod=0;
          Wire.write(160);
          Wire.endTransmission();
        }
      } else if(cur==128) readMethod = 1;
      else if(readMethod==0){
        switch(cur&0b11100000){
          case (START+DIGI_READ):
              if(cur&31<=19) digitalRequest(cur&31);
              else if(cur&32>=24) digitalPush(16+((cur&6)>>1),cur&1);
            break;
